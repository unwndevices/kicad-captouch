...
**Status:** Track A (Phases 6–7) **complete**; Track B (Phases 8–9) **complete**; Track C (Phases 10–13) **complete** — milestone done. Compiled 2026-06-18; Phase 9 landed 2026-06-19; Phase 10 landed 2026-06-19; Phase 11 landed 2026-06-19; Phase 12 landed 2026-06-19; Phase 13 landed 2026-06-20. Builds on [`plan.md`](./plan.md) (Phases 0–5 **complete**: a feature-complete electrodes-only generator for sliders, wheels, and XY diamond trackpads, with a live GUI, vendor presets, fab-rule guards, and standalone binaries).
...
| **13 — KiCad Action Plugin (IPC)** ✅ | A `pcbnew`/IPC plugin that runs the engine and places the generated footprint into the **open board** — resolving v1's deferred "insert directly" question. Packaged for the KiCad Plugin Manager. | **Done (with a corrected mechanism — see note):** an IPC Action Plugin (`kicad-python`/kipy) connects to the running KiCad, reads the open board's project, and launches the live GUI in *install mode*; **Add to KiCad project** writes the byte-identical footprint+symbol into the project's `captouch` library and registers it (`${KIPRJMOD}` in the project tables, or a personal **global** library; footprint `.pretty` and symbol `.kicad_sym` independently targetable), placeable via KiCad's Add Footprint/Add Symbol pickers. A shared `engine.build_widget`/`export_widget` dispatch keeps the install byte-identical to the CLI/GUI export. Ships as an installable `kicad-plugin/` bundle (manifest validated against KiCad's `api.v1` schema; PNG toolbar icons; `entry.py` shim) — **not** bundled into the PyInstaller binary, since IPC plugins run in KiCad's own managed venv (resolves §6). Headlessly tested: library install/merge/registration, manifest shape, project-dir resolution, `main()` dispatch. The live kipy connection + Qt loop + "passes DRC once placed" are the manual in-KiCad acceptance step (no live KiCad in CI). |
...
- ~~**Plugin distribution**~~ — **Resolved (Phase 13): a separate installable bundle, not bundled into the binary.** An IPC plugin runs inside KiCad's *own* managed Python virtualenv (KiCad creates it and `pip install`s the plugin's `requirements.txt`), which is incompatible with a frozen PyInstaller executable — so the plugin ships as the standalone [`kicad-plugin/`](../kicad-plugin/) directory a user drops into KiCad's plugins folder (the path to a future KiCad Plugin Manager / PCM listing). The standalone binary stays the offline, KiCad-independent option.
- **(Phase 13 note) "Place into the open board" → "install into the project library".** The original Phase 13 row assumed the IPC API could drop the generated footprint straight onto the board. It cannot: `kicad-python`/kipy builds footprints **item-by-item as protobuf** and offers no way to ingest an existing `.kicad_mod` (and `open_library_item` only opens the footprint *editor*). Re-authoring the emitter against the API would fork the single source of truth and has uncertain support for the embedded zones (Phase 8) and custom-polygon pads. So the plugin installs the **real, byte-identical** generated files into a KiCad library and registers it, and the user places them with KiCad's Add Footprint/Add Symbol pickers — the same end result (a placed widget that matches the standalone output and passes DRC), on the **stable** API, with the text emitter still authoritative.
